...
void getForwardPattern( const set<Word> & P, set<Word> & seed, int select )
{
...
Word SuperBox( Word x )
{
    ...
    Word z = SmallBitPerm( Nibble_to_Word( y0, y1, y2, y3 ) );
...
void generateSuperDDT( int ** table )
{
    for ( int i = 0; i < 65536; i++ )
        for ( int j = 0; j < 65536; j++ )
            table[i][ SuperBox( j ) ^ SuperBox( i ^ j ) ] ++;
}
/*
